...
contract NFTBurnAndMint is CCIPReceiver, OwnerIsCreator {
    ...
    function burnAndSendNFT(
        uint256 tokenId,
        address newOwner,
        uint64 source_chain_select,
        address reveiver) public returns(bytes32){
       //tansfer the wnft from owner to the pool
        wnft.transferFrom(msg.sender,address(this),tokenId);
        wnft.burn(tokenId);
        // send request to Chainlink CCIP to send the NFT to the other Chain
        bytes memory data = abi.encode(tokenId,newOwner);
        bytes32 messageId  = sendMessagePayLINK(source_chain_select,reveiver,data);
        return messageId;
    }
